...
def train_model(seed, train_loader, valid_loader):
    ...
    writer = utils.get_tensorboard_writer(f'fit_fdh_0106_incp3hab_57_seed{seed}')
    losses = []
    ...
    torch.save(net.state_dict(), f'models/fit_fdh_incp3hab_57_seed{seed}.pkl')
    np.save(f'results/val_losses_incp3hab_57_seed{seed}.npy', losses)
...
def main():
    ...
    for seed in range(1, 6):
        plt.plot(np.load(f'results/val_losses_incp3hab_57_seed{seed}.npy'), label=f"Seed {seed} Loss")
