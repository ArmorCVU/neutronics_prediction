...
# 定义不同形状的标记
markers = ['o', 's', '^', 'v', '<', '>', 'p', '*', '+', 'x']
labels = ['No attention', 'CAM', 'SAM', 'HAM'] #修改标签的名称
...
for idx, i in enumerate([1, 2, 3, 4]):
    seeds = []
    for j in range(5):
        seeds.append(np.load(f'results/val_losses_cbc_0109_{i}_seed{j+1}.npy')) #修改数据地址
    ...
    upper_bound = data_mean + data_std * 1 #增大阴影面积
    lower_bound = data_mean - data_std * 1 #增大阴影面积
...
# plt.title('The Generalization of Diverse Attention Mechanisms on $CBC$ Over Epochs', fontsize=16)
plt.xlim(0, 50) # 主图的epoch范围
plt.ylim(0,0.9) # 主图的loss范围
plt.tick_params(axis='both', which='major', labelsize=16) # 设置坐标轴数字的字体大小
plt.legend(fontsize=18)
plt.grid(True)
plt.savefig('画图/att.pdf')
