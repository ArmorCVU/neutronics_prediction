...
def create_model():
    net = nn.Sequential(
        Model(include_top=False, num_inception_resnet=2, input_channel=5),
        nn.Flatten(),
...
def train_model(seed, train_loader, valid_loader):
    ...
    writer = utils.get_tensorboard_writer(f'fit_fdh_0106_incp2_seed{seed}')
    losses = []
    ...
    torch.save(net.state_dict(), f'models/fit_fdh_0106_incp2_seed{seed}.pkl')
    np.save(f'results/val_losses_0106_incp2_seed{seed}.npy', losses)
...
def main():
    ...
    for seed in range(1, 6):
        plt.plot(np.load(f'results/val_losses_0106_incp2_seed{seed}.npy'), label=f"Seed {seed} Loss")
