...
from attention import HABLayer
...
class MixedOp(nn.Module):
  ...
  def __init__(self, C, stride, use_residual=False, use_attention=False):
  ...
  def forward(self, x, weights):
    ...
    if temp1.shape[2] == x.shape[2]:
      ...
      if temp1.shape[-1]==4:
        ans = torch.cat([temp1,self.mp(xtemp2)[:,:,1:,1:]], dim=1)
      else:
        ans = torch.cat([temp1,self.mp(xtemp2)], dim=1)
    ans = channel_shuffle(ans,self.k)
    
    if self.use_residual:
      # print(ans.shape, x.shape)
      if ans.shape[-1]!=x.shape[-1]:
        ans = ans + x[:,:,:ans.shape[-1], :ans.shape[-1]]
      else:
        ans = ans + x
    if self.use_attention:
        ans = self.hab(ans)
  ...
  def __init__(self, steps, multiplier, C_prev_prev, C_prev, C, reduction, reduction_prev,
    ...
    for i in range(self._steps):
      for j in range(2+i):
        ...
        op = MixedOp(C, stride, use_residual, use_attention)
        self._ops.append(op)
...
class Network(nn.Module):
  ...
  def __init__(self, C, num_classes, layers, criterion, steps=4, multiplier=4, stem_multiplier=3, use_residual=False, use_attention=False):
    ...
    self.stem = nn.Sequential(
      nn.Conv2d(5, C_curr, 3, padding=1, bias=False),
    ...
    for i in range(layers):
      if i in [layers//3, 2*layers//3]:
        ...
        reduction = False
      cell = Cell(steps, multiplier, C_prev_prev, C_prev, C_curr, reduction, reduction_prev, use_residual, use_attention)
      reduction_prev = reduction
      self.cells += [cell]
      C_prev_prev, C_prev = C_prev, multiplier*C_curr

    self.global_pooling = nn.AdaptiveAvgPool2d(1)
    self.regressor = nn.Linear(C_prev, num_classes) # 修改为回归输出
  ...
  def forward(self, input):
    ...
    out = self.regressor(out.view(out.size(0),-1)) # 使用线性回归输出
    return out

  def _loss(self, input, target):
    logits = self(input)
    # return self._criterion(logits.squeeze(), target) # 修改为回归损失
    return self._criterion(logits.reshape(target.shape), target)  # 修改为回归损失
